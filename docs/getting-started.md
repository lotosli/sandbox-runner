...
./sandbox-runner run --json-summary --config configs/run.local.sample.yaml
./sandbox-runner run --config configs/run.local.output.sample.yaml
...
./sandbox-runner run --json-summary --config configs/run.local.sample.yaml
./sandbox-runner run --config configs/run.local.output.sample.yaml
./sandbox-runner validate --config configs/run.local.sample.yaml --policy configs/policy.sample.yaml
./sandbox-runner k8s render-job --config configs/run.local.sample.yaml --policy configs/policy.sample.yaml
./sandbox-runner doctor
./sandbox-runner --version
```
...
```bash
./sandbox-runner run --config configs/run.local.sample.yaml
(cd dist && ./sandbox-runner-darwin-amd64 run --config ../configs/run.local.sample.yaml)
```
...
- [`configs/run.local.sample.yaml`](../configs/run.local.sample.yaml): baseline local sample
- [`configs/run.local.output.sample.yaml`](../configs/run.local.output.sample.yaml): local sample that emits visible stdout and stderr lines
...
- [`configs/run.minikube.sample.yaml`](../configs/run.minikube.sample.yaml): Minikube sample
- [`configs/run.k3s.sample.yaml`](../configs/run.k3s.sample.yaml): K3s sample
- [`configs/run.microk8s.sample.yaml`](../configs/run.microk8s.sample.yaml): MicroK8s sample
- [`configs/run.minikube.isolated.sample.yaml`](../configs/run.minikube.isolated.sample.yaml): Minikube isolated-mode smoke sample
- [`configs/run.k3s.isolated.sample.yaml`](../configs/run.k3s.isolated.sample.yaml): K3s isolated-mode smoke sample
- [`configs/run.microk8s.isolated.sample.yaml`](../configs/run.microk8s.isolated.sample.yaml): MicroK8s isolated-mode smoke sample
- [`configs/run.opensandbox.sample.yaml`](../configs/run.opensandbox.sample.yaml): OpenSandbox default runtime
- [`configs/run.opensandbox.isolated.sample.yaml`](../configs/run.opensandbox.isolated.sample.yaml): OpenSandbox isolated-mode sample

Sample file names are platform-first. When a platform exposes more than one user-facing mode, the second segment names that mode, such as `isolated`. Internally, the config still maps to `backend/provider/runtime_profile`. For `k8s` samples, `execution.provider` names the cluster flavor while `k8s.provider` keeps the legacy bridge value that existing render and submit commands still consume. `runtime.class_name` is the generic runtime-class hook for conditional profiles such as `kata` and `microvm`. These samples target the current K8s flow: `validate`, `k8s render-job`, and `k8s submit-job`.

Example isolated-mode smoke run on a local K8s provider:

```bash
./sandbox-runner validate --config configs/run.k3s.isolated.sample.yaml --policy configs/policy.sample.yaml
./sandbox-runner k8s submit-job --config configs/run.k3s.isolated.sample.yaml --policy configs/policy.sample.yaml
